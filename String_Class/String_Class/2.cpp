...
void MyString::reserve(int size) {
        ...
        // 만일 예약하려는 size가 현재 capacity보다 적다면
        // 아무것도 안해도 된다 
    }
...
int MyString::compare(const MyString& str) const {
    ...
    // 1은 (*this)가 사전식으로 더 뒤에 온다는 의미, 0은 문자열
    ...
    // 만일 문자열 길이가 딜다면 두 문자열은 아예 다른 문자열이 됨

    if (string_length == str.string_length) return 0;

    // 참고로 abc와 adcd의 크기 비교는 abcd가 더 뒤에오게 됨
...
MyString& MyString::insert(int loc, const MyString& str) {
    ...
    // abc 라는 문자열에 insert(1, "d")를 하게 된다면 adbc가됨
    ...
    if (string_length + str.string_length > memory_capacity) {
        ...
        //일단 insert되는 부분 직전까지의 내용을 복사
        int i;
        for (i = 0; i < loc; i++) {
            string_content[i] = prev_string_content[i];
        }

        //이제 새롭게 insert 되는 문자열을 넣음
        for (int j = 0; j != str.string_length; j++) {
            string_content[i + j] = str.string_content[j];
        }

        // 이제 다시 원 문자열의 나머지 뒷 부분을 복사
...
int main() {
    ...
    std::cout << "Location of second <very> in the string : " << str.find(str.find(0, "very") + 1, "very") << std::endl;
