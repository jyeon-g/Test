...
int main() {
	...
	while (1) {	// 컴퓨터는 0을 거짓, 0이 아닌 값을 참으로 판별, 조건문이 1이므로 언제나 참 // 무한루프 (break하면 멈춤)
		printf("도서 관리 프로그램 \n");
		printf("메뉴를 선택하세요 \n");
		printf("1. 책을 새로 추가하기 \n");
		printf("2. 책을 검색하기 \n");
...
int search_book(char(*book_name)[30], char(*auth_name)[30], char(*publ_name)[30], int num_total_book) {
	...
	if (user_input == 1) {
		// i가 0부터 num_total_book까지 가면서 각각의 책 제목을 사용자가 입력한 검색어와 비교하고 있음
		...
		// i부터 0까지 num_total_book까지 가면서 각각의 지은이 이름을 사용자가 입력한 검색어와 비교
		...
		// i부터 0까지 num_total_book까지 가면서 각각의 출판사를 사용자가 입력한 검색어와 비교 
